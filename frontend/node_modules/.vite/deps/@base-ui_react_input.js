...
var FieldsetRootContext = React.createContext(void 0);
...
function useFieldValidation(params) {
  ...
  const validationCommitIdRef = React4.useRef(0);
  const registerInput = React4.useCallback((element) => {
    if (!element) {
      return void 0;
    }
  ...
  const getValidationProps = React4.useCallback((disabled, externalProps = {}) => mergeProps(getDescriptionProps(externalProps), state.valid === false && !state.disabled && !disabled ? {
    "aria-invalid": true
...
function useFieldControlRegistration(params) {
  ...
  const activeFieldControlSourceRef = React5.useRef(null);
  const registrationRef = React5.useRef(null);
  const fallbackControlRef = React5.useRef(null);
...
var FieldRootInner = React6.forwardRef(function FieldRootInner2(componentProps, forwardedRef) {
  var _a;
  const {
    errors,
  ...
  const disabledFieldset = (_a = useFieldsetRootContext(true)) == null ? void 0 : _a.disabled;
  const validate = useStableCallback(validateProp || (() => null));
  const disabled = disabledFieldset || disabledProp;
  const [touchedState, setTouchedUnwrapped] = React6.useState(false);
  const [dirtyState, setDirtyUnwrapped] = React6.useState(false);
  const [filled, setFilled] = React6.useState(false);
  const [focused, setFocused] = React6.useState(false);
  const dirty = dirtyProp ?? dirtyState;
  const touched = touchedProp ?? touchedState;
  const markedDirtyRef = React6.useRef(dirty);
  const registeredFieldIdRef = React6.useRef(void 0);
  const [registeredFieldName, setRegisteredFieldName] = React6.useState();
  ...
  const getRegisteredFieldId = React6.useCallback(() => registeredFieldIdRef.current, []);
  const setRegisteredFieldId = React6.useCallback((id) => {
  ...
  const [validityData, setValidityData] = React6.useState({
  ...
  React6.useImperativeHandle(actionsRef, () => ({
...
var FieldRoot = React6.forwardRef(function FieldRoot2(componentProps, forwardedRef) {
...
var FieldItemContext = React7.createContext({
  disabled: false
});
if (true) FieldItemContext.displayName = "FieldItemContext";
function useFieldItemContext() {
  const context = React7.useContext(FieldItemContext);
  return context;
}

// node_modules/@base-ui/react/field/label/FieldLabel.mjs
var FieldLabel = React8.forwardRef(function FieldLabel2(componentProps, forwardedRef) {
  const {
    render,
    className,
    style,
  ...
  const labelRef = React8.useRef(null);
...
var FieldError = React9.forwardRef(function FieldError2(componentProps, forwardedRef) {
  const {
    render,
    id: idProp,
    className,
  ...
  const errorRef = React9.useRef(null);
  const [lastRenderedMessage, setLastRenderedMessage] = React9.useState(null);
  const [lastRenderedMessageKey, setLastRenderedMessageKey] = React9.useState(null);
  ...
  if (Array.isArray(error2)) {
    errorMessage = error2.length > 1 ? (0, import_jsx_runtime3.jsx)("ul", {
      children: error2.map((message) => (0, import_jsx_runtime3.jsx)("li", {
        children: message
      }, message))
    }) : error2[0] ?? "";
  }
...
var FieldDescription = React10.forwardRef(function FieldDescription2(componentProps, forwardedRef) {
  const {
    render,
    id: idProp,
    className,
...
var FieldControl = React11.forwardRef(function FieldControl2(componentProps, forwardedRef) {
  const {
    render,
    className,
    id: idProp,
...
var FieldValidity = function FieldValidity2(props) {
  ...
  const combinedFieldValidityData = React12.useMemo(() => getCombinedFieldValidityData(validityData, invalid), [validityData, invalid]);
  const isInvalid = combinedFieldValidityData.state.valid === false;
  const {
    transitionStatus
  } = useTransitionStatus(isInvalid);
  const fieldValidityState = React12.useMemo(() => {
    return {
...
var FieldItem = React14.forwardRef(function FieldItem2(componentProps, forwardedRef) {
  const {
    render,
    className,
    style,
  ...
  const controlId = hasParentCheckbox ? checkboxGroupContext == null ? void 0 : checkboxGroupContext.parent.id : void 0;
  const fieldItemContext = React14.useMemo(() => ({
    disabled
  }), [disabled]);
  const element = useRenderElement("div", componentProps, {
    ref: forwardedRef,
