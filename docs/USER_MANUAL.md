...
* **Jadwal & Kelas (`/staff/schedules`)**: Melihat dan mengatur jadwal pertemuan.
* **Tagihan & Pembayaran (`/staff/invoices`)**: Validasi transfer manual siswa dan menandainya lunas.
...
* **Jadwal Mengajar (`/teacher`)**: Menampilkan daftar kelas yang ditugaskan kepada Anda. Anda bisa mengklik kelas tersebut (buka `ClassDetailPage`) untuk melihat murid. Dari sana, Anda masuk ke pertemuan (`MeetingDetailPage`) untuk melakukan absen, memasukkan jurnal materi, dan memberikan nilai akhir (Grades).
...
* **Dashboard (`/student`)**: Ringkasan pengumuman.
